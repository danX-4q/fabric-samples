...
setGlobals() {
  ...
  if [ $ORG -eq 1 ]; then
    ...
    if [ $PEER -eq 0 ]; then
      ...
      CORE_PEER_ADDRESS=peer1.org11.example.com:8051
    fi
  elif [ $ORG -eq 2 ]; then
    ...
    if [ $PEER -eq 0 ]; then
      CORE_PEER_ADDRESS=peer0.org22.example.com:9051
    else
      CORE_PEER_ADDRESS=peer1.org22.example.com:10051
    fi
...
joinChannelWithRetry() {
  ...
  if [ $res -ne 0 -a $COUNTER -lt $MAX_RETRY ]; then
    ...
    echo "peer${PEER}.org${org_dn_part} failed to join the channel, Retry after $DELAY seconds"
    sleep $DELAY
    joinChannelWithRetry $PEER $ORG
  else
    COUNTER=1
  fi
  verifyResult $res "After $MAX_RETRY attempts, peer${PEER}.org${org_dn_part} has failed to join channel '$CHANNEL_NAME' "
}
