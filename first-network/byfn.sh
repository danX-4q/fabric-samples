...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    ...
    for PEER in peer0.org11.example.com peer1.org11.example.com peer0.org22.example.com peer1.org22.example.com; do
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/org11.example.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/org22.example.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
function generateCerts() {
  ...
  echo
  echo "Generate CCP files for Org11 and Org22"
  ./ccp-generate.sh
}
...
function generateChannelArtifacts() {
  ...
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for Org11MSP..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for Org22MSP   ##########"
  echo "#################################################################"
  set -x
  ...
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for Org22MSP..."
    exit 1
  fi
  echo
}
